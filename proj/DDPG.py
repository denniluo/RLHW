...
class Actor(object):
    def __init__(self, sess, action_dim, action_bound, learning_rate, t_replace_iter, S, S_):
        self.sess = sess
    ...
    def add_grad_to_graph(self, a_grads, GLOBAL_STEP):
...
class Critic(object):
    def __init__(self, sess, state_dim, action_dim, learning_rate, gamma, t_replace_iter, a, a_, S, S_, R, GLOBAL_STEP):
        self.sess = sess
...
def main():
    ...
    critic = Critic(sess, STATE_DIM, ACTION_DIM, LR_C, GAMMA, REPLACE_ITER_C, actor.a, actor.a_, S, S_, R, GLOBAL_STEP)
    ...
    for i_episode in range(MAX_EPISODES):
        ...
        while True:
            ...
            if done:
                ...
                logger.record_tabular('pos', int(env.unwrapped.hull.position[0]))
                logger.dump_tabular()
                logger.info('')
                break
